			...
			<article <?php post_class(); ?>>
				<header class="entry-header">
					...
					if ( 'post' === get_post_type() ) :
						...
						$cats = wp_get_post_categories(get_the_ID(), $cat_args); //Khai Lu
							...
							<div class="flex-fill text-right mr-2">
								...
								foreach ($cats as $value) {
									$badge_attrs = array(
										...
										'link'      => get_category_link($value->term_id) //Khai Lu
									);
									if(function_exists('sc_pix_badge')){
										echo sc_pix_badge($badge_attrs);
									}else{
										?>
								            <a href="<?php echo esc_url(get_category_link($value->term_id)); //Khai Lu?>"> 
